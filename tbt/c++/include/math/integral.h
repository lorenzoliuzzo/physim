...
class Integral {
    ...
    private: 
        ...
        void set_range(double a, double b) { m_a = a; m_b = b; }
        ...
        void set_sum(double x = 0) { m_sum = x; }
    ...
    public: 
        ...
        void begin_integration(double a, double b, unsigned int n = 1000, double sum0 = 0) {
            check_range(a, b); 
            reset_integral(); 
            set_steps(n); 
            set_h(); 
        ...
        virtual void integrate(double a, double b, const FunctionBase& f, unsigned int n = 1000, double fmax = 0) = 0; 
        
        virtual void integrate_fixed(double a, double b, const FunctionBase& f, double prec = 1.e-5, double fmax = 0) = 0; 
        
        void error_integral(double new_old) { m_error = 4 * new_old / 3.; } 
        ...
        void print_integral(double precision = 1.e-5) {
...
class Midpoint : public Integral {
    ...
    public: 
        ...
        void integrate(double a, double b, const FunctionBase& f, unsigned int n = 1000, double fmax = 0) override {
            begin_integration(a, b, n); 
            for (unsigned int i{}; i < m_steps; i++) { m_sum += (f.eval(m_a + (i + 0.5) * m_h)); }
            m_integral = m_sign * m_sum * m_h; 
        }

        void integrate_fixed(double a, double b, const FunctionBase& f, double prec = 1.e-5, double fmax = 0) override {
