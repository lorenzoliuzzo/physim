...
class ODE(Position, ABC) :
    ...
    def rk4(self, h = 0.001, t = None, pos = None) :
        ...
        appo = np.empty(shape = (3, 2), dtype = float)  
        k0 = self.eval(t, self.get_position())
        k1 = self.eval(t + h / 2, self.get_position() + k0 * h / 2)
        k2 = self.eval(t + h / 2, self.get_position() + k1 * h / 2)
        k3 = self.eval(t + h, self.get_position() + k2 * h)   
        appo = h * (k0 + 2 * k1 + 2 * k2 + k3) / 6
        self.set_position(appo[:, 0], appo[:, 1])
